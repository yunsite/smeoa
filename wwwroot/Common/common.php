		...
		foreach($tree as $val) {
			...
			if(isset($val['_child'])){
				if (is_array($val['_child'])) {
					$list = array_merge($list, tree_to_list($val['_child'],$level+1));
				}
			}
		}
		return $list;
	...
	if (is_array($tree)){ 
	  	...
	  	foreach($tree as $val){    
		   	if(isset($val["name"])) {
				...
				if (isset($val['_child'])){
			   		$html=$html."<li>\r\n<a node=\"$id\" href=\""."$url\"><i class=\"icon level$level\"></i><span>$title</span></a>\r\n";
	...
	if (is_array($tree)){ 
	  	...
	  	foreach($tree as $val){    
		   	if(isset($val["name"])) {
				...
				if (isset($val['_child'])){
			   		$html=$html."<li>\r\n<a node=\"$id\" ><i class=\"icon level$level\"></i><span>$title</span></a>\r\n";
...
function todo_status($status) {
	...
	$objPHPExcel->getProperties()->setCreator("小薇")
								 ->setLastModifiedBy("小薇")
